...
type Backend interface {
	...
	//
	// Дескриптор переходит во владение Backend: он закроет его в Down или
	// прямо в Up, если тот не удался, и только его. Хост, которому tun нужен
	// дольше устройства, отдаёт копию (dup) и держит оригинал открытым.
