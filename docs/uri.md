...
| `listen` | `-listen` | локальный `ip:port` для WireGuard. |
...
```json
{"v":1,"provider":"vk","peer":"1.2.3.4:56000","transport":"tcp",
...
Параметры из URI переопределяют базовые флаги (`-peer`, `-transport`, `-obf-key`, `-n` ...).
